...
// Define a service and UUID: The "numbers" are randomly generated from https://www.uuidgenerator.net/
BLEService tempService("208c9c6f-dcf8-4c1f-8a43-8f1674c21d6e");

// Characteristics for turning the fan on or off, setting the current threshold temperature, and sending the temperature to any connected web application
...
// Update the temperature characteristic's value and notify all connected apps
...
//Read the current temperature from the temperature sensor and call method to display value on OLED screen
void readTemperature() {
...
//Based on the temperature threshold, turn the fan on if the sensor reads a higher temperature.
...
//Interpret data sent by a web app to change the temperature threshold
void changeTargetTemp (int temperature, int decimal){
   targetTemperature  = temperature/1.0 + decimal/10.0;
}
